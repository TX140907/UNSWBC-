...
public:
    ...
    void observe(Controller& ct, Game& game) {
        ...
        openSea = w == 64 && h == 64 && mapPlan() == MapPlan::BigEmpty;
        if (openSea) {
            // Help has fast pearl respawns; only enable open-map foraging
            // after observing a respawn interval that cannot occur there.
            openSea = std::any_of(cells.begin(), cells.end(), [](const Cell& c) {
                return c.seen >= 0 && c.due - c.seen > 20;
            });
        }
        ...
        for (auto const& tile : ct.get_tiles()) if (auto part = tile.get_dragon()) {
            ...
            for (int d = 0; d < 4; ++d) {
                ...
                if (w <= 20 && h <= 20) risk = (part->get_team() == ct.get_team() ? 250.0 : 300.0)
                    / (options * options);
                else risk *= std::clamp(ct.get_length() / 8.0, 1.0, 4.0);
                if (w <= 20 && h <= 20 && part->get_team() != ct.get_team()) risk *= w == 11 ? 0.35 : 0.65;
                if (w == 16 && h == 16 && ct.get_team() == Team::B &&
                    round >= 400 && actualLength > 4 && mapPlan() == MapPlan::Colosseum)
                    risk *= std::min(4.0, actualLength / 3.0);
                danger[q] += risk;
    ...
    bool activeForaging() const {
        return (w == 32 && unswbc::ct && unswbc::ct->get_team() == Team::B) || (w == 25 && h == 25) ||
            (openSea && unswbc::ct && unswbc::ct->get_team() == Team::B) ||
            (w == 60 && h == 40 && unswbc::ct && unswbc::ct->get_team() == Team::A);
    }
        ...
        for (int depth = 1; depth <= horizon; ++depth) {
            ...
            if (depth == horizon) {
                ...
                for (auto const& leaf : next) {
                    ...
                    double value = leaf.rank + ((w > 20 || h > 20) && w != 32 ? 4.0 : 2.0) * std::min(area(leaf), int(leaf.body.size()) + 8);
                    if (w > 20 || h > 20) value += escapeValue(leaf);
                    if (!exits) value -= 500;
    ...
    Policy policy(bool confined) const {
        Policy p; p.expand=p.workers=((w<=20 && h<=20) || confined);
        if (w <= 20 && h <= 20) {
            p.expand = p.workers = true;
            p.target = 64; p.stop = 500; p.continuation = 0.35;
            if (w == 11) p.continuation = 0.7;
            if (mapPlan() == MapPlan::DefaultSmall) { p.stop = 280; p.continuation = 0.55; }
            if (mapPlan() == MapPlan::Colosseum)
                p.stop = unswbc::ct && unswbc::ct->get_team() == Team::B ? 470 : 500;
            return p;
        }
        switch (mapPlan()) {
        ...
        case MapPlan::Default:
            p.expand = true; p.workers = true; p.cautiousChild = false;
            p.splitLength = 4; p.target = 48; p.stop = 300;
            break;
        case MapPlan::Queen:
            if (unswbc::ct && unswbc::ct->get_team() == Team::A) {
                p.expand = true; p.workers = false;
                p.splitLength = 6; p.target = 24; p.stop = 250;
            }
            break;
        case MapPlan::Schooltime:
            if (unswbc::ct && unswbc::ct->get_team() == Team::A) {
                p.expand = true; p.workers = false; p.cautiousChild = true;
                p.splitLength = 6; p.target = 24; p.stop = 180;
            }
            break;
        // Preserve the established length advantage on these maps first.
        case MapPlan::BigEmpty: p.expand=p.workers=false; break;
        case MapPlan::Colosseum: case MapPlan::Compact:
        case MapPlan::Generic: break;
        }
        return p;
    }
    int childRoom() const {
    ...
    bool trophyLeader(const Controller& ct) const {
        // Reserve a third of each block of ids for growth, on either team.
        // Once any dragon grows beyond breeding size, preserve it as well.
        return ct.get_id() % 6 < 2 || ct.get_length() > 4;
    }
    // Only enemy heads are ram targets; hitting a body merely loses our dragon.
    int sacrificeDirection(const Controller& ct) const {
        if (w != 25 || h != 25 || ct.get_length() != 2) return -1;
        if (trophyLeader(ct) || round >= 300 || ct.get_unit_count() <= 6) return -1;
        const int head = index(ct.get_position());
        const auto distance = [&](int a, int b) {
            int dx = std::abs(a % w - b % w), dy = std::abs(a / w - b / w);
            return std::min(dx, w - dx) + std::min(dy, h - dy);
        };
        int pearlDistance = n + 1;
        for (int p = 0; p < n; ++p)
            if (cells[p].pearl && round - cells[p].seen <= 20)
                pearlDistance = std::min(pearlDistance, distance(head, p));
        if (pearlDistance == n + 1) return -1;

        std::vector<char> targets(n, 0);
        for (auto const& tile : ct.get_tiles()) {
            auto part = tile.get_dragon();
            if (part && part->is_head() && part->get_team() != ct.get_team()) {
                int p = index(tile.get_position());
                if (2 * distance(head, p) < pearlDistance) targets[p] = 1;
            }
        }
        // Find a known clear route; only its final enemy-head tile may be occupied.
        std::vector<int> first(n, -1);
        std::deque<int> queue{head};
        first[head] = 4;
        while (!queue.empty()) {
            int p = queue.front(); queue.pop_front();
            for (int d = 0; d < 4; ++d) {
                int q = graph[p][d];
                if (q < 0 || !visible[q] || first[q] >= 0 || contains(body, q)) continue;
                if (cells[p].edge[d] == -2 || (fixed[q] && !targets[q])) continue;
                first[q] = p == head ? d : first[p];
                if (targets[q]) return first[q];
                queue.push_back(q);
            }
        }
        return -1;
    ...
    std::vector<int> choose(Controller& ct) {
        ...
        int sacrifice = sacrificeDirection(ct);
        if (sacrifice >= 0) {
            // Reconstruct from the next observation, including growth on approach.
            body.clear(); complete = false;
            return {sacrifice};
        }
        const bool splitInHalfMap = w == 25 && h == 25;
        const bool leader = splitInHalfMap && trophyLeader(ct);
        const int splitTarget = std::min(40,
            std::max(0, w - 2) * std::max(0, h - 2) / 3);
        const auto canSplit = [&](int childSize) {
            return ct.can_split(childSize) && (!splitInHalfMap ||
                ct.get_unit_count() < splitTarget);
        };
        // Grow dedicated leaders early, then let every survivor grow in endgame.
        const int trophyBreedingStop = ct.get_team() == Team::A ? 180 : 260;
        if (splitInHalfMap && !leader && round < trophyBreedingStop && ct.get_length() == 4 && canSplit(2)) {
            if (complete) body.resize(2);
            else body.clear();
            return {-1};
        }
        ...
        const bool breeding = splitInHalfMap ? round < trophyBreedingStop :
            (round < plan.stop || ((confined || plan.cautiousChild) &&
        ...
        if (!splitInHalfMap && plan.expand && breeding && ct.get_length() >= plan.splitLength &&
            ...
            ((w <= 20 && h <= 20) || (complete && breedingExit(body.front()) && breedingExit(body.back())))) {
            if (complete) body.resize(body.size() - 2);
            else body.clear();
            return {-1};
        }
        if (!leader && plan.workers && breeding && ct.get_length() <= 5) {
        ...
        State selected;
        int bestDepth = -1;
        const int horizon = w == 32 && h == 32 ? 18 :
            ct.get_team() == Team::A && h == 35 ? 26 :
            ct.get_team() == Team::A && h == 40 ? 22 :
            mapPlan()==MapPlan::Schooltime ? 18 : LEVIATHAN_HORIZON;
        for (int d = 0; d < 4; ++d) {
            State first;
        ...
        const int rescueDepth = w == 60 && h == 40 && ct.get_team() == Team::A ? 3 : 8;
        if (bestDepth < rescueDepth && complete && body.size() >= 4 &&
            canSplit(ct.get_length() - 2)) {
            State child;
        ...
        if (bestDepth < 3 && canSplit(2) && complete) {
        ...
        if (action.empty()) {
            ...
            if (!complete && canSplit(ct.get_length() - 2)) {
                int childSize = ct.get_length() - 2;
                body.clear(); complete = false;
                return {-1, childSize};
            }
