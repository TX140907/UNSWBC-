...
public:
    ...
    bool reproduce(Controller& ct, bool confined) const {
        ...
        if (n <= 256) return round < 350 && ct.get_unit_count() < std::min(32,n/3);
        const int units = ct.get_unit_count();
        const int target = confined ? 40 : 56;
        if (units >= target || round >= 480) return false;
        // Once reserves exist, leave a subset of long dragons to grow.
        if (units >= 12 && ct.get_length() >= 8 && (myid % 4 == 0 || round >= 420)) return false;
        if (round >= 420 && units >= 12) return false;
        int food = 0;
        for (int p = 0; p < n; ++p) if (visible[p] && cells[p].pearl && !fixed[p]) ++food;
        const int threshold = confined ? 4 : (food >= 5 && units < 12 ? 4 : 6);
        return ct.get_length() >= threshold && complete &&
            breedingExit(body.front()) && breedingExit(body.back()) && nurserySpace() >= 6;
    }
    std::vector<int> choose(Controller& ct) {
        ...
        const bool breeding = n <= 256 ? round < 350 :
            (round < 420 || (round < 480 && ct.get_unit_count() < 12));
        ...
        State selected;
        int bestDepth = -1;
        const int horizon = KRAKEN_HORIZON;
        for (int d = 0; d < 4; ++d) {
            State first;
