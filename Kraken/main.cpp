...
#ifndef KRAKEN_HORIZON
#define KRAKEN_HORIZON 14
#endif
#ifndef KRAKEN_BEAM_WIDTH
#define KRAKEN_BEAM_WIDTH 24
#endif
...
public:
        ...
        for (auto const& tile : ct.get_tiles()) if (auto part = tile.get_dragon()) {
            ...
            for (int d = 0; d < 4; ++d) {
                ...
                if (n <= 256) risk = (part->get_team() == ct.get_team() ? 250.0 : 300.0)
                    ...
                    risk *= std::clamp(ct.get_length() / 7.0, 1.0, 5.0);
                    if (ct.get_unit_count() < 6 && ct.get_length() >= 8) risk *= 1.5;
                }
                risk *= kraken_policy(w, h, team).risk_percent / 100.0;
                danger[q] += risk;
                for (int dd = 0; dd < 4; ++dd) {
                    int r = graph[q][dd];
                    if (r >= 0 && !fixed[r] && !bodyMask[r])
                        danger[r] += ct.get_length() >= 8 ? risk * 0.18 / std::max(1,options) : 6;
                }
            }
        }
    ...
    bool advance(const State& s, int d, State& next, int depth, bool sprint = false) const {
        ...
        double gain = grows ? (visible[q] ? 100 : 65) * kraken_policy(w, h, team).food_percent / 100.0 : 0;
        if (sprint) gain -= 125;
    ...
    Evaluation search(State start, int horizon = KRAKEN_HORIZON) const {
        std::vector<State> beam{std::move(start)};
        Evaluation out;
        constexpr int WIDTH = KRAKEN_BEAM_WIDTH;
    ...
    // Space available to a newborn while its stationary parent's body remains.
    // Unknown terrain is not counted as proven nursery space.
    int nurserySpace() const {
        if (!complete || body.size() < 4) return 0;
        std::vector<char> blocked(n,0);
        for (int p : body) blocked[p] = 1;
        std::deque<int> queue{body.back()};
        int count = 0;
        while (!queue.empty() && count < 12) {
            int p = queue.front(); queue.pop_front(); ++count;
            for (int d = 0; d < 4; ++d) {
                int q = graph[p][d];
                if (q < 0 || cells[p].edge[d] == -2 || cells[q].seen < 0 ||
                    blocked[q] || fixed[q] || danger[q] >= 180) continue;
                blocked[q] = 1; queue.push_back(q);
            }
        }
        return count;
    }
    bool reproduce(Controller& ct, bool confined) const {
        if (!ct.can_split(2)) return false;
        const auto policy = kraken_policy(w, h, team);
        if (n <= 256) return round < policy.small_stop_round &&
            ct.get_length() >= policy.small_split_length && ct.get_unit_count() < std::min(policy.small_target,n/3);
        if (!confined && !policy.open_expand) return false;
        const int units = ct.get_unit_count();
        const int target = confined ? policy.confined_target : policy.open_target;
        if (units >= target || round >= policy.stop_round) return false;
        // Once reserves exist, leave a subset of long dragons to grow.
        if (units >= policy.reserve && ct.get_length() >= 8 && (myid % 4 == 0 || round >= policy.growth_round)) return false;
        if (round >= policy.growth_round && units >= policy.reserve) return false;
        int food = 0;
        for (int p = 0; p < n; ++p) if (visible[p] && cells[p].pearl && !fixed[p]) ++food;
        const int threshold = confined ? policy.confined_split_length :
            (food >= 5 && units < policy.reserve ? policy.rich_split_length : policy.split_length);
        return ct.get_length() >= threshold && complete &&
            breedingExit(body.front()) && breedingExit(body.back()) && nurserySpace() >= policy.nursery;
    }
    std::vector<int> choose(Controller& ct) {
        ...
        const auto policy = kraken_policy(w, h, team);
        const bool breeding = n <= 256 ? round < policy.small_stop_round :
            (round < policy.growth_round || (round < policy.stop_round && ct.get_unit_count() < policy.reserve));
        ...
        if (reproduce(ct, confined)) {
            if (complete) body.resize(body.size() - 2);
            else body.clear();
            return {-1};
        }
        if (breeding && ct.get_length() <= 5) {
            ...
            for (int d = 0; d < 4; ++d) {
                ...
                double score = (growAt(p, root) ? 250 : 0) + 150.0 / (foodDist[p] + 1)
                    ...
                    - 0.5 * std::min(visits[p], 12) + policy.worker_continuation / 100.0 * continuation;
        ...
        State selected;
        int bestDepth = -1;
        const int horizon = policy.search_horizon;
        for (int d = 0; d < 4; ++d) {
            State first;
